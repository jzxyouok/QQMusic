...
@interface DXLrcView() <UITableViewDataSource>
...
{
    // 1.创建Cell
    DXLrcCell *cell = [DXLrcCell lrcCellWithTableView:tableView];
    ...
    // 2.给cell设置数据
    // 2.1.取出模型
    DXLrcline *lrcline = self.lrclist[indexPath.row];
...
- (void)setLrcName:(NSString *)lrcName
{
    ...
    // 2.解析歌词
    self.lrclist = [DXLrcTool lrcToolWithLrcName:lrcName];
...
- (void)setCurrentTime:(NSTimeInterval)currentTime
{
    ...
    for (int i = 0; i < count; i++) {
        // 1.拿到i位置的歌词
        DXLrcline *currentLrcLine = self.lrclist[i];
        
        // 2.拿到下一句的歌词
        NSInteger nextIndex = i + 1;
        DXLrcline *nextLrcLine = nil;
        if (nextIndex < count) {
            nextLrcLine = self.lrclist[nextIndex];
        }
        
        // 3.用当前的时间和i位置的歌词比较,并且和下一句比较,如果大于i位置的时间,并且小于下一句歌词的时间,那么显示当前的歌词
        if (((self.currentIndex != i) || (i == 0 && self.currentIndex == 0)) && currentTime >= currentLrcLine.time && currentTime < nextLrcLine.time) {
        ...
        if (self.currentIndex == i) {
            // 4.1.拿到i位置的cell
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:i inSection:0];
            DXLrcCell *cell = (DXLrcCell *)[self.tableView cellForRowAtIndexPath:indexPath];
...
- (void)generatorLockImage
{
    ...
    // 2.拿到三句歌词
    // 2.1.获取当前的歌词
    DXLrcline *currentLrc = self.lrclist[self.currentIndex];
    // 2.2.上一句歌词
    NSInteger previousIndex = self.currentIndex - 1;
    DXLrcline *prevousLrc = nil;
    if (previousIndex >= 0) {
        prevousLrc = self.lrclist[previousIndex];
    }
    // 2.3.下一句歌词
    NSInteger nextIndex = self.currentIndex + 1;
    DXLrcline *nextLrc = nil;
