...
#import "DXPlayingViewController.h"
#import "Masonry.h"
...
#define DXColor(r,g,b) ([UIColor colorWithRed:r/255.0 green:g/255.0 blue:b/255.0 alpha:1.0])

@interface DXPlayingViewController () <UIScrollViewDelegate, AVAudioPlayerDelegate>
...
// 歌词的View
@property (weak, nonatomic) IBOutlet DXLrcView *lrcView;

// 歌词的Label
@property (weak, nonatomic) IBOutlet DXLrcLabel *lrcLabel;
...
@implementation DXPlayingViewController
...
- (void)viewWillLayoutSubviews
{
    ...
    self.iconView.layer.borderColor = DXColor(36, 36, 36).CGColor;
}
...
{
    // 1.取出当前播放歌曲
    DXMusic *playingMusic = [DXMusicTool playingMusic];
    ...
    // 3.开始播放歌曲
    AVAudioPlayer *currentPlayer = [DXAudioTool playMusicWithMusicName:playingMusic.filename];
...
- (IBAction)previous {
    // 1.取出上一首歌曲
    DXMusic *previousMusic = [DXMusicTool previousMusic];
...
- (IBAction)next {
    // 1.取出下一首歌曲
    DXMusic *nextMusic = [DXMusicTool nextMusic];
    
    // 2.播放下一首歌曲
    [self playingMusicWithMusic:nextMusic];
}

- (void)playingMusicWithMusic:(DXMusic *)music
{
    // 1.停止当前歌曲
    DXMusic *playingMusic = [DXMusicTool playingMusic];
    [DXAudioTool stopMusicWithMusicName:playingMusic.filename]; 
    
    // 3.播放歌曲
    [DXAudioTool playMusicWithMusicName:music.filename];
    
    // 4.将工具类中的当前歌曲切换成播放的歌曲
    [DXMusicTool setPlayingMusic:music];
